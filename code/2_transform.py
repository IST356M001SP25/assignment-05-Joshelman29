...
def process_salary_survey():
    ...
    surveys_data['_country'] = surveys_data['What country do you work in?'].apply(pl.clean_country_usa)

    # lookup the state code from the state name
    survey_states_combined = surveys_data.merge(state_data, left_on="If you're in the U.S., what state do you work in?", right_on='State', how='inner')
